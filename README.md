...
죄송합니다. 요청하신 내용을 잘못 이해했습니다. 기존의 `README.md` **양식을 그대로 유지**하면서, 이전에 합쳐서 보여드렸던 \*\*8번(주요 성과/트러블 슈팅)\*\*과 **9번(회고)** 내용을 **추가**하여 최종 `README.md` 파일을 생성해 드리겠습니다.

아래는 기존 내용을 유지하고 8번과 9번 섹션을 추가한 최종 결과입니다.
...
7. <a href="#-7-프론트엔드백엔드-상세">📎 7. 프론트엔드/백엔드 상세</a><br>

**8. <a href="#-8-백엔드-주요-성과-및-트러블-슈팅">🔥 8. 백엔드 주요 성과 및 트러블 슈팅</a><br>**
**&nbsp;&nbsp;8.1 <a href="#81-성능-최적화-결과-redis--k6">성능 최적화 결과 (Redis & k6)</a><br>**
**&nbsp;&nbsp;8.2 <a href="#82-트러블-슈팅-1-redis-캐싱-후-cpu-병목-해결">트러블 슈팅 1: Redis 캐싱 후 CPU 병목 해결</a><br>**
**&nbsp;&nbsp;8.3 <a href="#83-트러블-슈팅-2-복합-지역-필터-로직-구현">트러블 슈팅 2: 복합 지역 필터 로직 구현</a><br>**

**9. <a href="#-9-프로젝트-회고-kpt">💡 9. 프로젝트 회고 (KPT)</a><br>**
</details>
...
## 3.2 아키텍처
안정적인 서비스 제공을 위해 AWS EC2 인스턴스에 백엔드를 배포하고, **Redis를 캐시 계층으로 두어 데이터베이스 부하를 분산**했습니다.
![architecture](readme-assets/3-2_architecture.png)
...
\-\> 상세 가이드는 'frontend' 폴더 내 [README 파일](https://www.google.com/search?q=frontend/README.md%23%EF%B8%8F-%EB%A1%9C%EC%BB%AC-%EA%B0%9C%EB%B0%9C-%ED%99%98%EA%B2%BD-%EC%84%B8%ED%8C%85)에서 확인
...
\-\> 상세 가이드는 'backend' 폴더 내 [README 파일](https://www.google.com/search?q=backend/README.md%23-3-%EC%84%9C%EB%B2%84-%EC%8B%A4%ED%96%89)에서 확인
...
| 이름 (GitHub) | 역할 | 주요 담당 기능 |
| :---: | :---: | :--- |
| \<a href="https://github.com/sieuns" target="\_blank"\>양시은\</a\> | 팀장 | 중계 일정 관리, 통합 검색 기능, 마이페이지 |
| \<a href="https://github.com/kevinmj12" target="\_blank"\>김민제\</a\> | 부팀장 | 회원가입/로그인, 식당 관련 기능, 지도 관련 기능 |
| \<a href="https://github.com/neverleaveualong" target="\_blank"\>심우현\</a\> | 팀원 | 즐겨찾기, 비밀번호 초기화, 중계 일정 관련 부가 기능 |

## 6.2 백엔드

| 이름 (GitHub) | 역할 | 주요 담당 기능 |
| :---: | :---: | :--- |
| \<a href="https://github.com/jo-eunchae" target="\_blank"\>조은채\</a\> | 팀원 | 더미데이터 생성, 식당 관련 API, K6 부하테스트 코드 작성, Redis 캐싱 |
| \<a href="https://github.com/hwanga12" target="\_blank"\>황가연\</a\> | 팀원 | **중계 일정 / 검색 관련 API, K6 부하테스트 / Redis 캐싱, AWS EC2 배포** |
| \<a href="https://github.com/HwangJieun03" target="\_blank"\>황지은\</a\> | 팀원 | DB 셋팅 / S3 이미지 서버, 유저 / 즐겨찾기 관련 API, K6 부하테스트 / Redis 캐싱 |

# 📎 7. 프론트엔드/백엔드 상세

기타 상세는 [프론트엔드 README 파일](https://www.google.com/search?q=frontend/README.md), [백엔드 README 파일](https://www.google.com/search?q=backend/README.md)을 참고하세요
...
**빈번한 조회**와 **복잡한 필터링**이 필요한 **중계 일정** 및 **통합 검색** API에 **Redis 캐싱 전략**을 도입하여 성능 병목을 해결하고 트래픽 처리량 극대화.
